...
class CofiPara(torch.nn.Module):
    def __init__(self, args, params = None, **kwargs) -> None:
        ...
        # self.loss_bist = nn.CrossEntropyLoss(weight=torch.FloatTensor([1,3]))
        self.loss_bist = nn.CrossEntropyLoss(weight=torch.FloatTensor([3,1]))
    ...
    def forward_train_cls(self, text = None, image = None, input_ids = None,attention_mask = None, labels_cls=None, labels=None, return_dict=True, src_out = None, batch = None):
        ...
        memory_multit_txt   = torch.concat([memory_txt_cls, self.proj_layer_v(torch.cat([memory_V,memory_V_res],dim=1)), self.proj_layer_m1(sequence_output)], dim=1)
   
    

       
        # logits_multi2 = self.cls_multi2(self.multi_cls_layer2(memory_multit_txt).view(bs, -1))
        logits_multi2 = self.cls_bi(self.multi_cls_layer_bi(memory_multit_txt).view(bs, -1))
        ...
        loss_multi2 = self.loss_bist(logits_multi2, label_multi2.sum(dim=1).long())#[8,2], [8]
        # pdb.set_trace()

      
        # pdb.set_trace()
        outputs['loss_multi2'] = loss_multi2
    ...
    def forward_cls(self, text = None, image = None, input_ids = None,attention_mask = None,labels=None, labels_cls=None, return_dict=True, src_out = None, batch=None):
        ...
        # logits_multi = self.cls_multi(self.multi_cls_layer(memory_multi1).view(bs, -1))
        ...
        return logits, img_tgt_dict, logits_multi, logits_multi2
