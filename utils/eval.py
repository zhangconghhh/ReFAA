...
def batch_post_process_cls(labels, labels_pred, labels_bi, em_0, em_1, gt_0, gt_1 ):
    ...
    ems = []
    accs = []
    label_bi = torch.argmax(labels_bi, dim=1)

    for lab, lab_pred, lab_bi in zip(labels, labels_pred, label_bi):
        lab_sp = lab.split(' ')
        pred = torch.zeros(len(lab_sp))
        if lab_bi == 1:
            pred[ torch.argmax(lab_pred[:len(lab_sp)], dim=1) == 2] = 1
        gt = torch.zeros(len(lab_sp))
        gt[[a== 'O' for a in lab_sp]] =1
        # pdb.set_trace()
        ...
        if gt.sum() == 0:
            ...
            gt_1  += 1
        if sum(exact_match) // len(exact_match) == 1:
            if gt.sum() == 0:
                em_0 += 1
            else:
                em_1 += 1
    ...
    return preds, gts, batch_acc, batch_EM, em_0, em_1, gt_0, gt_1
...
def evaluate_text_dgm4(preds, gts):
...
def eval_iou_batch(pred_boxes,target_boxes):
    IOU_pred, IOU_50, IOU_75, IOU_95 = [], [], [], [], 
    boxes1 = pred_boxes #(output_coord)
    boxes2 = target_boxes #(fake_image_box)

    IOU, _ = box_iou(boxes1, boxes2, test=True)
    ...
    return IOU_score, IOU_ACC_50, IOU_ACC_75
...
def loss_by_feat_single_zc(cls_scores: Tensor, bbox_preds: Tensor,text_masks: Tensor,
                        ...
                        #                             loss_weight=1.0,
                        #                             class_weight=None),
                        Loss_cls = CrossEntropyLoss(ignore_index=0),
    ...
    label_weights = None    
    # pdb.set_trace()
    loss_cls = Loss_cls(cls_scores.squeeze().cpu(), box_labels.squeeze().long(), label_weights)#, avg_factor=cls_avg_factor)
    ...
    return loss_cls, loss_bbox, loss_iou#, loss_img



def loss_by_feat_single(cls_scores: Tensor, 
                        # pred_img: Tensor,
                        bbox_preds: Tensor,text_masks: Tensor,
                        box_labels: Tensor, target_boxes: Tensor,
    ...
    # cls_labels = torch.zeros(cls_scores.shape[:2]).to(cls_scores.device).long()
    # cls_labels[:,0][box_labels[:,0]==1] = 1
    # cls_labels[:,1][box_labels[:,0]==2] = 1   
    # pdb.set_trace()
    # loss_img = Loss_img(pred_img, box_labels[:,0].to(pred_img.device).long())
                        #cls_labels.view(-1))#,  
    ...
    cls_scores = torch.masked_select(cls_scores, text_mask1).contiguous() # [6,2,512] 
