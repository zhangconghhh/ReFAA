...
class GDinoWithDomainLN(nn.Module):
    ...
    def save_state_dict(self, args, epoch):
        ...
        torch.save({'model':self.state_dict()}, save_name)

                            
    def save_pretrained(self, save_dir: str):
        """
        与 HF/PEFT 接口兼容的保存函数。
        会在 save_dir 下写入权重和一个简单的 config。
        """
        os.makedirs(save_dir, exist_ok=True)
        # 权重
        torch.save({'model': self.state_dict()},
                os.path.join(save_dir, 'pytorch_model.bin'))
        # 可选：写一点最小配置，方便 from_pretrained 或调试
        cfg = {
            'class': 'GDinoWithDomainLN',
            'num_domains': getattr(self, 'num_domains', None)
        }
...
def trainer(model, args):
    ...
    wrapped = get_peft_model(wrapped, config)
    ...
    while epoch < args.epochs:
        ...
        # wrapped.save_pretrained(args.save_path)
        wrapped = PeftModel.from_pretrained(wrapped, args.save_path)
        # model = model.merge_and_unload(       
        wrapped.merge_and_unload().save_state_dict(args, epoch)
