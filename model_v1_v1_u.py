...
class CofiPara(torch.nn.Module):
    ...
    def forward_train_cls(self, text = None, image = None, input_ids = None,attention_mask = None, labels_cls=None, labels=None, return_dict=True, src_out = None, batch = None, task_id=None):
        # encode text
    ...
    def forward_cls(self, text = None, image = None, input_ids = None,attention_mask = None,labels=None, labels_cls=None, return_dict=True, src_out = None, batch=None):
        ...
        np.save('memory_visual_sc.npy',np.array(memory_visual.cpu().detach().numpy())) 
        np.save('memory_res_sc.npy',np.array(memory_res.cpu().detach().numpy())) 
        pdb.set_trace()
        if self.args.stage == "stage2":
        ...
        np.save('biheat_sc.npy',np.array(heat.cpu().detach().numpy())) 
        pdb.set_trace()
